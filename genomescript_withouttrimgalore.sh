...
bwa mem  -R "@RG\tID:${Samplename}\tSM:${Samplename}\tLB:Lib\tPU:PU\tPL:Illumina" \
 ...
 "${Samplename}"_R2.fastq.gz | samtools sort -@ "${threads}" -o BamFiles/"${Samplename}".bam & 
done < Sample_Names.txt
wait
...
mosdepth -x -t 20 --by ${genome_bed} QC/AlignmentQC/Mosdepth/Exome/${Samplename}_Exome BamFiles/${Samplename}_Mduplicates.bam \
