...
for filename in $filenames; do
  ...
  for field in "${fields[@]}"; do
    if [[ $field =~ ^[0-9]{6}(P|P1|P2|P3|P4|P5|P6|M|F)$ ]]; then
      ...
      sampledir=$(find "$instdir" -maxdepth 1 -type d -name "${sampleID//[[:alpha:]]/}" -print -quit)
      ...
      if [[ -z $(find $sampledir -type f -name '*.tsv') && -n $(find $sampledir -type f -name "$sampleID*.fastq.gz") ]]; then
         ...
         (cd $sampledir && sbatch exomescript-v4-withouttrimgalore.sh &) || { echo "sbatch failed"; exit 1; }
      else
